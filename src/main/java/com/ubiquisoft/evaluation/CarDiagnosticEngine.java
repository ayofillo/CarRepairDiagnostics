...
public class CarDiagnosticEngine {
    ...
    public void executeDiagnostics(Car car) {
         ...
         */
        if(car==null){
            System.err.println("--- No car detail provided, exiting...");
            System.exit(1);
        }

        boolean allGood = true;
        System.out.println("Starting diagnostics check...\n");
        // running first diagnostics
        System.out.println("*** Running first diagnostics, ensuring car data fields are present ***");
        ...
        //running second diagnostics
        if (allGood) {
            System.out.println("*** First diagnostics successful... ***");
            System.out.println("*** Running second diagnostics, ensuring no part is missing ***");
            Map<PartType, Integer> missingParts = car.getMissingPartsMap();
            for (PartType pt : missingParts.keySet()) {
                printMissingPart(pt, missingParts.get(pt));
                allGood = false;
            }
        }

        //running third diagnostics
        if (allGood) {
            System.out.println("*** Second diagnostics successful... ***");
            System.out.println("*** Running third diagnostics, ensuring all parts are in working condition ***");
